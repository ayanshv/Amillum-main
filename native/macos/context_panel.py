...
class ContextPanel:
    ...
    def build(self):
        ...
        workbench=AK.NSButton.buttonWithTitle_target_action_('Add to Workbench',self.target,'full:')
        workbench.setBezelStyle_(AK.NSBezelStyleRounded); workbench.setEnabled_(True)
        workbench.setToolTip_('Choose a finding in Selection review, then confirm what to save.')
        workbench.setFrame_(AK.NSMakeRect(20,18,180,30)); root.addSubview_(workbench)
        note=AK.NSTextField.labelWithString_('Review before saving')
