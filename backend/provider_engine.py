...
def _get_client():
    ...
    if _CLIENT is None:
        ...
        if not key:
            raise ValueError(
                "GEMINI_API_KEY is missing. Add it to a .env file in the project root or set it in your environment, on the backend server."
            )
        _CLIENT = genai.Client(api_key=key, http_options={"timeout": 45000, "retry_options": {"attempts": 1}})
    return _CLIENT
